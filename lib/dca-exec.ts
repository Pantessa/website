...
export async function runDcaTurn(
  ...
  if (create) {
    ...
    if (!chainId) {
      return { reply: `📆 I can't find ${create.buyToken} on a first-class chain's official token list (Base, Ethereum, Arbitrum, Optimism, Arc, Robinhood Chain) — name the chain, e.g. "buy $${create.buyUsd} of ${create.buyToken} ${cadenceLabel(create.cadence)} on robinhood".` }
    }
