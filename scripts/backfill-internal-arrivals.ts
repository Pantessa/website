...
const RULES: Rule[] = [
  ...
  { table: 'broker_intents', id: 'B1', note: "harness identities/bylines (agent or agent_key LIKE 'harness%', 'Harness Agent', incl. the M4 organic-probe keys)", where: Prisma.sql`(coalesce(agent, '') ILIKE 'harness%' OR coalesce(agent_key, '') ILIKE 'harness%' OR agent_key_hash = ${agentHandleFor('harness-desk-key')})` },
  { table: 'broker_intents', id: 'B2', note: "QA adversarial-drive identities (agent or agent_key LIKE 'qa-%' — the 2026-08-18 squad drives that ran minutes before the stamped build)", where: Prisma.sql`(coalesce(agent, '') ILIKE 'qa-%' OR coalesce(agent_key, '') ILIKE 'qa-%')` },
  {
    table: 'broker_intents',
    id: 'B3',
    note: 'the pre-stamp desk era (created before 2026-08-19): the desk stamp (DeskCallOpts) landed 2026-08-18; BROKER_DESK_ENABLED was never set on prod, so every earlier row — incl. null-identity rows and the hungry-agent/payer-agent/Portfolio Agent/Rebalancer demo fixtures — came from our own local runs against the shared DB',
    where: Prisma.sql`created_at < '2026-08-19'`,
  },
