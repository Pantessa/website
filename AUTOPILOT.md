...
2. **Never**: merge any PR, push to `main`, force-push, deploy, publish,
   run `db:ingest --prune`, or make live paid x402 calls. No spending of any
   kind. Schema changes must be additive (`prisma db push`, no resets).
3. **Verify before PR**: `npx tsc --noEmit` + `npm run build` minimum. UI items
...
4. **Honesty**: anything unverifiable headlessly (wallet signatures, SIWE-gated
   UI states) is flagged "needs manual pass" in the PR — never claimed as done.
5. **Logging**: after each item, update the Progress log below on the
...
6. **Isolation**: work in this session's worktree only. Don't touch other
   branches, other sessions' work, or open PRs from earlier today.
7. **Stop conditions**: queue exhausted, two consecutive failures, or owner
...
- [ ] **1. API keys for headless agents** — `ApiKey` model (sha256-hashed
  ...
  script (mint → authed request → ledger row → cleanup).
...
- [ ] **5. Receipts → `Message.meta`** — chat client persists the receipts
