...
// Pangolin path data, shared with components/Logo.tsx. Generated geometry —
// see public/brand/pangolin/README.md for the construction and regeneration.
const PANGOLIN_BODY =
...
const PANGOLIN_SNOUT = 'M 95.7 55.5 L 123.2 61.8 L 103.4 43.2 Z'
const PANGOLIN_CUTS: [string, number][] = [
...
export function YeetfulMark({ size = 22 }: { size?: number }) {
  // Pantessa's own pangolin mark — armour cut into plates by grout seams
  // (same geometry as components/Logo.tsx and
  // public/brand/pangolin/pantessa-pangolin.svg). Used for the first-party
  // `yeetful-tool-*` internal MCPs so they carry the brand mark instead of a
  // bare Archivo "Y" lettermark.
  //
  // Single-ink here: the set plate takes `currentColor` rather than the accent,
  // so the mark inverts with its tile like every other vendored mark in this
  // file — the grout gaps keep the plates legible either way. The eye is
  // punched through the mask so it stays a hole on a light OR dark tile.
  // A per-instance mask id keeps multiple marks on one page from colliding.
  const maskId = `pt-pango-${useId().replace(/:/g, '')}`
  return (
    <svg viewBox="0 0 128 128" width={size} height={size} fill="none" aria-hidden style={{ display: 'block' }}>
      <g transform="translate(6.86 12.04) scale(0.8877)">
        <mask id={maskId}>
          <rect x="-1" y="18.1" width="130.7" height="80.9" fill="#000" />
          <path d={PANGOLIN_BODY} fill="#fff" stroke="#fff" strokeWidth={5} strokeLinejoin="round" />
          <circle cx="100.9" cy="50.3" r="11" fill="#fff" />
          <path d={PANGOLIN_SNOUT} fill="#fff" stroke="#fff" strokeWidth={8} strokeLinejoin="round" />
          <g fill="none" stroke="#000" strokeLinejoin="round" strokeLinecap="round">
            {PANGOLIN_CUTS.map(([d, w]) => (
              <path key={d} d={d} strokeWidth={w} />
            ))}
          </g>
          <circle cx="101" cy="54.7" r="4.6" fill="#000" />
        </mask>
        <rect x="-1" y="18.1" width="130.7" height="80.9" fill="currentColor" mask={`url(#${maskId})`} />
      </g>
    </svg>
  )
}
