...
import { sealArt } from '../lib/seal-geometry'
...
export function YeetfulMark({ size = 22 }: { size?: number }) {
  // Pantessa's own mark — the Open Seal (guilloché turning, open at the
  // heart; see lib/seal-geometry.ts). Used for the first-party
  ...
  return (
    <svg viewBox="0 0 128 128" width={size} height={size} fill="none" aria-hidden style={{ display: 'block' }}>
      {art.circles.map((c) => (
        <circle key={`c${c.r}`} cx={64} cy={64} r={c.r} fill="none" stroke="currentColor" strokeWidth={c.w} opacity={c.op} />
      ))}
      {art.rings.map((r, i) => (
        <path key={i} d={r.d} fill="none" stroke="currentColor" strokeWidth={r.w} opacity={r.op} />
      ))}
    </svg>
  )
}
