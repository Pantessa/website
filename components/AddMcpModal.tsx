...
export default function AddMcpModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // form, closed by a tap outside, a swipe, Escape or back.
  const phone = usePhonePosture()
  // The form and its footer, shared by both postures (the Sheet on a phone,
  // the modal at lg+).
  const body = (
    <>
    {submitted ? (
    ...
    // The Sheet's own body scrolls on a phone: no second scroller inside it.
    <div className={phone ? 'px-5 py-4 space-y-4' : 'max-h-[68vh] overflow-y-auto px-5 py-4 space-y-4'}>
      {/* URL + discover */}
      <div>
      ...
      {status === 'guest' && (
        <p className="text-[11px] leading-relaxed text-[color:var(--muted)] rounded-xl border border-[var(--line)] bg-[var(--surf-1)] px-3 py-2.5">
          ...
          <button
            ...
            // A text link inside the hint: on a phone it keeps its look under
            // a 44px hit area (squad mobile-native, CHAT r3 — QA measured 90×18).
            className="text-[color:var(--accent)] hover:underline font-medium max-lg:relative max-lg:before:absolute max-lg:before:content-[''] max-lg:before:-inset-x-1 max-lg:before:-inset-y-[13px]"
            data-hit-44=""
          >
            Connect &amp; sign in
          </button>
  ...
  // On a phone the footer's action is THE button of the sheet: full width,
  // 48px (squad mobile-native, CHAT r3 — QA measured "Request review" 143×40).
  const footer = (
    <>
      ...
      {submitted ? (
        <button
          type="button"
          onClick={onClose}
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold bg-white text-zinc-950 hover:bg-zinc-200 transition-colors max-lg:w-full max-lg:min-h-12 max-lg:justify-center max-lg:text-[14px]"
        >
          Done
        </button>
      ) : (
  ...
  if (phone) {
    return (
      ...
      >
        <p className="px-5 pt-1 mono text-[11px] text-[color:var(--muted-2)]">discovered from the server · reviewed by a Pantessa partner · then routes in chat</p>
        {body}
      </Sheet>
    )
  }
